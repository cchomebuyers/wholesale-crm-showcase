# Pilot Manifest — Owner / Tax-Roll Connector — Orange County, CA (FIPS 06059)

**Status:** DRAFT CONNECTOR-READY (GLM iter 191, 2026-06-28)
...
**Pilot county rationale:** Orange County CA is the **#2 raw-harvest source by volume (1,985 rows, `orange-ca-parcels`)** per `data/source-registry/property-intelligence-status.md`, population rank #6 nationwide (3.2M). The existing OCGIS parcel layer (`https://ocgis.com/arcpub/rest/services/Map_Layers/Parcels/FeatureServer/0/query`) exposes only `SITE_ADDRESS` — `has_owner: false`, "no_owner_in_any_layer" (`layer-discovery-results.jsonl`, `owner-fieldmap-audit.jsonl`). `owner-join-plan-active-sources.md` explicitly flags: *"Orange CA | no | OCGIS parcel layer has only site address/geometry; need county assessor/tax-roll source."* This manifest is the lawful path to close that owner gap. **Non-duplicate** — the 8 existing verified tax-roll pilots (Maricopa AZ, Miami-Dade FL, Clark NV, Harris TX, Los Angeles CA, Santa Clara CA, Cook IL, Hillsborough FL) do not cover Orange CA.
...
|---|---|---|
| Owner + parcel + assessed roll | Orange County Assessor | `https://www.ocgov.com/assessor` |
| Tax-defaulted / power-to-sell list + auction | Orange County Treasurer–Tax Collector (TT-C) | `https://www.ttc.ocgov.com/` |
| Bulk open-data portal (Assessor parcels + roll) | OC Open Data / OC Public Works GIS | `https://data.ocgov.com/` , `https://www.ocgis.com/` |
...
- OC Open Data bulk download: `https://data.ocgov.com/` → Assessor "Secured Assessment Roll" / parcel feature layer (search "Assessor Parcel"). Prefer the bulk CSV/GeoJSON export over per-feature paging.
- Public-records request fallback: Assessor Public Service counter / online request form (`https://www.ocgov.com/assessor`) for the secured-roll extract (owner name field public; mailing address field withheld from bulk electronic redistribution per RTC §408.1).
- Existing OCGIS ArcGIS FeatureServer (`.../Parcels/FeatureServer/0`) is retained for **situs + geometry only** (owner not present — keep current `orange-ca-parcels` registry row as the situs/geometry join key, do not overwrite).

**B. Tax-defaulted property / power-to-sell list (delinquent amount + sale date):**
- TT-C "Tax Sales" / "Tax-Defaulted Property" page: `https://www.ttc.ocgov.com/` → Tax-Defaulted Properties / Power to Sell section. The TT-C posts the parcel roster (PDF/CSV or via its auction host) ahead of each sale under RTC §3701 publication requirements.
...
| `situs` (property address) | Assessor `SITUS` / `SITE_ADDRESS` (join to existing OCGIS `orange-ca-parcels` geometry) | Situs address on roster |
...
Join A↔B on `APN`; join to harvest `orange-ca-parcels` on `APN` or `SITE_ADDRESS`.
...
- Honor `robots.txt` on `ocgov.com`, `ttc.ocgov.com`, `data.ocgov.com`, `ocgis.com`.
...
Reject this pilot and pick an alternate CA county assessor/TT-C (LA already on file; Riverside, San Bernardino, Alameda, San Diego are the other uncovered top-10 harvest sources — see `owner-join-plan-active-sources.md`) if **any** is true:
...
1. Confirm live OC Open Data Assessor secured-roll dataset slug + bulk export format at `https://data.ocgov.com/` (confirm at onboarding — dataset slugs reorganize).
2. Confirm TT-C tax-defaulted-property roster page subpath + format (CSV/PDF) at `https://www.ttc.ocgov.com/`; record the auction-host domain for public sale-metadata reads only.
...
4. Map fields per §3; join A↔B on `APN`; join to `orange-ca-parcels` harvest rows on `APN`/`SITE_ADDRESS`.
...
- Pairs with existing `orange-ca-parcels` registry row (situs + geometry) — this manifest supplies the owner + APN + roll layer the parcel layer lacks.
- Same CA frame portability as `pilot-manifest-taxroll-losangeles-ca-2026-06-27.md` and `pilot-manifest-taxroll-santaclara-ca-2026-06-27.md` (CA RTC §408/§408.1/§2192 PII gating applies to all CA assessor rolls).
- Photo/imagery sidecar: pair with county appraiser parcel photos once Orange CA appraiser imagery endpoint is confirmed (mirrors Miami-Dade / Hillsborough FL photo pilots).
...
- Additional uncovered top-10 harvest counties (Riverside CA, San Bernardino CA, Alameda CA, San Diego CA, NYC NY) — promote on future loops if owner gap persists after Orange CA pilot build.
